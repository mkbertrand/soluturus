...
public final class SoluturusAddition {
	...
	private static Expression product_add(Product a1, Expression a2) {
		...
		for (BigInteger i = BigInteger.ZERO; i.compareTo(a1l) < 0; i = i.add(BigInteger.ONE)) {
			...
			Expression b = bcomps.size() == 1 ? bcomps.get(0)
					: new Product(bcomps.toArray(new Expression[bcomps.size()]));
			Expression c = ccomps.size() == 1 ? ccomps.get(0)
					: new Product(ccomps.toArray(new Expression[ccomps.size()]));
